  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <PromotionModal
        ...
        variant={activeAd?.submissionType === 'drop_sponsorship' ? 'drop_sponsorship' : 'ad'}
          ...
          <nav className="hidden md:flex items-center gap-1">
            {NAV.map(({ to, label, icon: Icon }) => (
              <Link
                key={to}
                to={to}
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm transition-colors no-underline ${pathname === to
                        ...
                        {(notifs ?? []).length === 0 ? (
                          ...
                          (notifs ?? []).map((n) => (
                            <div
                              ...
                              className={`flex items-start gap-3 px-4 py-3 cursor-pointer hover:bg-surface-2 transition-colors group ${!n.isRead ? 'bg-surface-2/50' : ''
                                }`}
                            >
                              <span
                                className={`mt-1.5 w-2 h-2 rounded-full shrink-0 ${n.isRead ? 'bg-surface-3' : (PRIORITY_DOT[n.priority] ?? 'bg-brand')
                                  }`}
                              />
        ...
        <nav className="md:hidden flex overflow-x-auto border-t border-surface-3 px-2 py-1 gap-1">
          {NAV.map(({ to, label, icon: Icon }) => (
            <Link
              key={to}
              to={to}
              className={`flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs whitespace-nowrap no-underline ${pathname === to
                ? 'bg-brand/15 text-brand'
      ...
      <footer className="border-t border-surface-3 py-4 text-center text-xs text-text-muted">
        &copy; 2026 Drauwper. Drauwp it when its hot. All rights reserved.
      </footer>
    </div >
  );
}
